/* hygen devices runtime — library/devices/runtime.js, bundled with engine/stage/runtime.js and every
   library/devices/<type>/device.js into build/assets/hygen/devices.js (engine/src/devices.ts → installDevices).

   A stage beat (library/scenes/CONTRACT.md, «Бит v2») is one sub-composition built by the engine: the stage
  ...
  window.HygenDevices = {
    ...
    mount: function (tl, cfg) {
      for (var i = 0; i < cfg.devices.length; i++) {
        ...
        if (!fn) throw new Error("hygen: устройство " + dev.type + " не загружено (library/devices/" + dev.type + "/device.js)");
