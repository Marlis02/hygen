...
## Где лежит
- `library/music/<стиль>-NN.wav` + `<стиль>-NN.license.json` рядом. Сейчас: `documentary-dark-01` — плато процедурного гула Помпей (CC0).
- Треки стиля перечислены в `library/styles/<стиль>/style.json → music.tracks`; там же громкость, приглушение, вход и выход по умолчанию.
...
- `"music": false` — без музыки. Так у Помпей: их гул — ручной звук сцен (`sound.events: false`).
...
## Проверка
`npm run build -- projects/<id> --no-render` — шаг «музыка» пишет `build/assets/music/bed.wav` и `build/music.json`; полная сборка — автопроверка `loudness` (−14 LUFS ±0,5).

## Тестовый трек с битом
`test-beat-100` — процедурный трек 100 BPM, 30 тактов 4/4 (72 с, длиннее Short — подложка не склеивается): бочка на каждую долю, хлопок на 2 и 4, хэты восьмыми, пэд Am–F–C–G. CC0, генерируется `python3 engine/py/make_test_beat.py library/music/test-beat-100.wav`. Только для проверки `sync: music` (proof `projects/_proof/typo`), не для публикации.
