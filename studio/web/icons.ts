// Tabler Icons 3.34.1 (MIT, © Paweł Kuna), outline set — inlined: the panel works without the network.
...
export function icon(name: string, size = 20): Element {
  const box = document.createElement("span");
  box.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" class="icon" aria-hidden="true">${PATHS[name] ?? ""}</svg>`;
