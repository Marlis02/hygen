import { api, clear, fail, h, mountJob, t, toast } from "../lib.ts";
  ...
  clear(
    ...
    h(
      ...
      h(
        ...
        h("div", { class: "panel" }, h("h3", null, t("settings.budget")), b.budget === null ? h("div", null, t("settings.noBudget", { spent: b.spent })) : [h("div", { class: "row between" }, h("span", null, t("settings.spent", { spent: b.spent, budget: b.budget })), h("b", null, t("settings.left", { left: b.left }))), h("div", { class: "meter", style: "margin:8px 0" }, h("div", { style: `width:${Math.min(100, (100 * b.spent) / Math.max(1, b.budget))}%` }))], h("div", { class: "muted small" }, b.since ? t("settings.since", { d: new Date(b.since).toLocaleString("ru-RU") }) : ""), h("table", { class: "plain", style: "margin-top:8px" }, h("tr", null, h("th", null, t("settings.project")), h("th", null, t("settings.takes")), h("th", null, t("settings.chars"))), (s.usage as Dict[]).filter((u) => u.takes).map((u) => h("tr", null, h("td", null, u.project), h("td", null, u.takes), h("td", null, u.chars))))),
