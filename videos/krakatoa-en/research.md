...
Пересказ по раскадровке-доказательству `videos/_proof/krakatoa-v2` на движке v2: механизм — как остров стал самым громким звуком в истории. Старая версия (сцены D3: счётчик → одометр → карта → шкала → картина → дробь) — `history/2026-09-14/`.
...
## Media
Из proof krakatoa-v2 (скопированы с license.json): `krakatoa-lithograph.jpg` (evidence, PD), `krakatoa-before.jpg` / `krakatoa-after.jpg` (place, PD), силуэты `engine/assets/maps/sunda-strait.svg`, `indian-ocean.svg` (Natural Earth).
