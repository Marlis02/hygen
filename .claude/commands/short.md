---
description: ИИ-режиссёр Short v3.4 (S2) — тема → исследование с источниками → концепция (look) → арка → сценарий → «что видит зритель» на каждый бит → intent + target + данные → текст на экране (субтитры, text.kinetic, sync) → медиа с ролью → грамматика → снимки → сборка MP4
argument-hint: "<тема, например: Krakatoa 1883> | <id проекта с brief.json>"
---

# /short — режиссёр Short (v3.4: stage + devices + intent + текст; своя папка, жанр, черновик на Kokoro)
