...
`hygen.config.json` (в git, образец `hygen.config.example.json`) — всё, кроме секретов: `voice` (provider, voiceId, model, kokoroVoice), `look`, `budgets.elevenlabsChars`, `bitrate` (crf, preset, maxrate, bufsize), `short` (целевая длина), `voice.charsPerSecond` (символов в секунду Kokoro и ElevenLabs — оценка длины реплик), `paths`, `studio.port`. Все команды и панель читают его; панель правит формой.
...
#    (или: панель → «Новый ролик» → бриф с жанром и бюджетом → «Создать» заведёт проект-бриф →
#     вкладка «Диалоги» → «Открыть диалог» — режиссёр получает /short <id> первой строкой)
...
| `npm run studio` | панель на http://localhost:5177 (только 127.0.0.1; `-- --no-open` — не открывать браузер): сервер + Vite одной командой |
| `npm run studio:build` · `npm run studio:dist` | собрать страницу панели в `studio/dist/` (не в git) · отдать собранное без Vite |
...
| `npm run studio:check` | смоук-тест панели: сервер и страница, каждый элемент библиотеки карточкой, форма бита для всех устройств и stage (Preact → строка), создание проекта из брифа (ничего не стартует само), сборка Помпей кнопкой на Kokoro с картой ролика (`-- --no-build` — без сборки) |
| `node engine/src/cli.ts timeline <проект> [--read] [--json]` | карта ролика `build/timeline.json` из готового `build/` (`--read` — как её видит панель, ничего не пишет) |
...
`npm run studio` → http://localhost:5177. Локальный Node-сервер (`studio/server.ts`, API — `studio/api.ts`) над движком, страница — Preact + @preact/signals на Vite (`studio/web/src`, TSX): в `npm run studio` Vite работает middleware того же сервера и порта, `npm run studio:build` собирает `studio/dist/`, `npm run studio:dist` отдаёт собранное. Строки интерфейса — `studio/i18n/ru.json` и `studio/i18n/en.json`, язык выбирается в «Настройках» (только панель, на ролики не влияет). API отвечает только на `localhost`/`127.0.0.1` и принимает запросы на запись только со страницы панели (заголовок `X-Hygen`) — в панели живёт терминал. После изменения путей движка панель перезапускают. Панель пишет те же файлы, что CLI и режиссёр: `project.json`, `media.json`, `library/music/music.json`, `library/looks/<id>/look.json`, `hygen.config.json` — ручная правка JSON работает как раньше.
...
| · Редактор | плеер сверху (переключатель «MP4 / Композиция»), под ним дорожки карты ролика — биты, голос (слова), субтитры, устройства, переходы, музыка (сетка долей и огибающая приглушения); курсор идёт за плеером, клик по дорожке — перемотка, протяжка по шкале — скраббинг, Ctrl+колесо — масштаб, наведение на шкалу — кадр из контактного листа; клик по биту — его форма справа (та же, что в «Битах»). «Композиция» — собранный `build/index.html` в iframe на паузе: кадр ставит рантайм HyperFrames (`renderSeek`) в браузере за ~25 мс, без рендера. Мышью: край бита — пауза после реплики (`pad[1]`, 0–3 с), левый край устройства — момент со снапом к слову своего бита, дорожка музыки — сдвиг `music.in` по сетке долей; Ctrl+Z / Ctrl+Shift+Z. Каждая правка пишет `project.json` через API бита и ставит «изменено, пересоберите», карта до пересборки — «оценка». Сборку редактор не запускает |
...
| Новый ролик | бриф: тема, id, жанр (история \| наука \| факты и списки \| развлекательное \| объясняющее), look или «пусть решит», длина, **бюджет ElevenLabs в символах** (обязателен, предзаполнен `voice.defaultBudgetChars`), пожелания; необязательные — арка, чего избегать, обязательный кадр. «Создать» только пишет `brief.json`, скелет `project.json` (статус «бриф», Kokoro, `voice.budgetChars`, без битов) и пустой `media.json` и открывает проект на вкладке «Диалоги» с брифом и кнопкой «Открыть диалог». Ни диалога, ни исследования, ни поиска медиа, ни сборки само не стартует |
...
| Режиссёр | список проектов (те же фильтры): выбрать ролик или «Новый ролик» — без проекта диалог не запускается; сверху — идущие диалоги с «Завершить» |
...
- диалог живёт на сервере: закрытие вкладки браузера его не завершает, «Завершить» спрашивает подтверждение. В сайдбаре — строка на каждый идущий диалог: клик ведёт в его «Диалоги», рядом «Завершить» — осиротевший диалог убивается откуда угодно;
- журнал — `projects/<id>/dialogs/<дата>.log` (экранный текст: строки, ушедшие с экрана терминала, без перерисовок и спиннеров) и `<дата>.json` рядом (сессия, длительность, число правок); «Продолжить» поднимает `claude --resume <id сессии>`;
