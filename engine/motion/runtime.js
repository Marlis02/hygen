/* hygen motion runtime — engine/motion/runtime.js, copied into build/assets/hygen/runtime.js.
   ...
   CSS and canvas only, no WebGL: the render stays on 4 workers (DECISIONS.md). Presets: engine/motion/*.json. */
(function () {
  "use strict";
  var TAU = Math.PI * 2;
  ...
  // ── camera (engine/motion/camera.json): state at video time t inside beat b ──
  ...
  // transitions are modules of engine/transitions/<id>/transition.js (appended to this file by the build):
  ...
  var root = {
    ...
    update: function (t) {
      ...
      // transition overlays (modules of engine/transitions)
      var mods = modules();
  ...
  // ── type (engine/motion/type.json): presets of the scene's text slots, inside the scene composition ──
