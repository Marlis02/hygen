...
function writeStoryboard({ spec, buildDir, timings, voices }: AssembleInput): void {
  ...
  const lines = [
    ...
    `# STORYBOARD — ${spec.id}`,
    "",
    "> Generated by the hygen engine from project.json on every build. Do not edit.",
    "",
  ];
...
function finalizeIndex({ spec, look, buildDir, timings, words, sound, scenes, music }: AssembleInput, style: StyleDef, captions: CaptionPlan): number {
  ...
  // look layers: transitions over the cuts (project.json, beat, look hit/default), media backgrounds, textures, motion runtime
