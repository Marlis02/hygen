...
const VALUE_FLAGS = new Set(["--voice", "--quality", "--mp4", "--params", "--tone", "--seed", "--at", "--look", "--textures", "--beat", "--stage", "--src", "--device", "--text", "--dur", "--preset"]);
...
async function main(argv: string[]): Promise<number> {
  ...
  if (cmd === "scene" && (value("--stage") || value("--device") || (positional[0] && isRecipe(positional[0])))) {
    const dur = value("--dur");
    const common = { beat: value("--beat"), look: value("--look"), text: value("--text"), dur: dur === undefined ? undefined : Number(dur), tone: value("--tone"), at: value("--at"), preset: value("--preset") };
